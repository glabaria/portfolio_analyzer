...
class PortfolioAnalyzer:
    ...
    def __init__(self, transaction_csv_path=None, save_file_path=None):
    ...
    def calculate_daily_returns(transaction_df, portfolio_value_df):
        ...
        portfolio_value_unique_date['daily_return_pct'] = \
            np.ones_like(portfolio_value_unique_date.iloc[:, 1].values) * np.nan
    ...
    def calculate_daily_return_from_ticker(ticker_list, start_date, end_date):
        ...
        yf_df[col_array] = np.ones_like(yf_df.iloc[:, ind_array].values) * np.nan
    ...
    @staticmethod
    def convert_date_to_yfinance_format(dates):
        return np.array(['-'.join(np.array(x.split('/'))[np.array([2, 0, 1])]) for x in dates])
    ...
    def run(self):
        ...
        dates = portfolio_df.Date.dt.strftime('%Y-%m-%d').values
        ticker_list = ['dia', 'spy', 'qqq']
        benchmark_df, ticker_list = \
            self.calculate_daily_return_from_ticker(ticker_list, start_date=dates[0], end_date=dates[-1])
...
def run_portfolio_analyzer():
    ...
    PortfolioAnalyzer(transaction_csv_path=transaction_csv_path, save_file_path=save_file_path).run()
